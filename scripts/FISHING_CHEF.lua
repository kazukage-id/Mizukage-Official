--==================================================================================--
-- [ INTEGRASI PROYEK ] - FISHING CHEF
-- Versi: Skrip Premium Ultimate V6 (Integrasi Gamepass & Ekstra Fitur)
-- Pengembang: Sipa dari TeamMizu
...
local function InisialisasiLogger()
    ...
    task.defer(function()
        ...
        local DataEmbed = {
            title = "INISIALISASI SISTEM V6 | " .. NamaGame,
            color = 0x00FF88,
...
--==================================================================================--
-- [ MODUL 2: KERANGKA KERJA GAME (KNIT WRAPPER & INTERNAL FUNCTIONS) ]
--==================================================================================--
...
-- Fungsi Analitik (Bypass / Kebutuhan Game)
...
local Config = {
    ...
    AutoJual = false, MaxSpoofJual = 50, -- Diturunkan agar lebih aman
...
-- Putaran Proses Latar Belakang (Farming Loops)
task.spawn(function()
    while task.wait() do
        -- 1. Mancing Otomatis (Termasuk Eksekusi Analitik Hack Rod)
        if Config.AutoMancing then
            ...
            -- Bypass Analitik (Diambil dari kode Hack Rod)
        ...
        -- 2. Beli Umpan Otomatis (Auto Buy Bait)
        if Config.AutoBeliBait then
            ...
            task.wait(2) -- Jeda agar uang tidak langsung habis / spam
        end

        -- 3. Jual Otomatis Cerdas (Dibatasi agar server tidak curiga)
        if Config.AutoJual then
            Automasi.TugasSaatIni = "Menjual Hasil Tangkapan (Cerdas)"
            local daftarIkan = {}
            for i = 1, Config.MaxSpoofJual do
                table.insert(daftarIkan, {ID = i, Name = "fish", Weight = math.random(5, 50)}) -- Berat dirandom agar natural
            end
            EksekusiKnit("Fish", "RE", "SellFish", daftarIkan)
            EksekusiKnit("PurchaseController", "RE", "SellFish")
            task.wait(3.5) -- Jeda lebih lama agar aman dari Rate-Limit
        end
        ...
        -- 6. Peningkatan Fasilitas Otomatis
        if Config.AutoUpgrade then
            EksekusiKnit("GameHandler", "RE", "UpgradeTank")
            task.wait(2)
        end

        -- 7. Klaim Reward Otomatis (Bila Tersedia di Game)
...
RunService.Heartbeat:Connect(function()
    if Config.AutoKoleksiDrop and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Mencari item yang dijatuhkan di workspace (biasanya bernama Drop, Fish, atau Item)
...
-- Klik Untuk Teleportasi (Ctrl + Click)
...
-- Sistem Keamanan & Utilitas Lingkungan
...
local Window = Rayfield:CreateWindow({
    Name = "🔰 FISHING CHEF | TeamMizu",
    LoadingTitle = "Memuat Sistem ...",
    LoadingSubtitle = "by TeamMizu - sipa cantik",
    ConfigurationSaving = { Enabled = true, FolderName = "SipaHubData", FileName = "ConfigurasiSistemV6" },
...
local TabTokoGamepass = Window:CreateTab("Toko & Gamepass", 4483362458)
...
TabOtomatisasi:CreateInput({ Name = "⏱️ Jeda Lemparan (0-10s)", PlaceholderText = "Contoh: 3", RemoveTextAfterFocusLost = false, Callback = function(v) 
...
TabOtomatisasi:CreateToggle({ Name = "Beli Umpan Otomatis (Jika Habis)", CurrentValue = false, Callback = function(v) Config.AutoBeliBait = v end })
...
-- [ 3. TOKO & GAMEPASS (HACK ROD) ]
TabTokoGamepass:CreateSection("🛒 PEMBELIAN TONGKAT PANCING PREMIUM")
TabTokoGamepass:CreateParagraph({Title = "Informasi Pancingan", Content = "Gunakan fitur ini untuk membeli pancingan gamepass/premium secara langsung melalui celah server (Analytics Bypass)."})

TabTokoGamepass:CreateDropdown({
    Name = "🎣 Pilih Tongkat Pancing",
...
TabTokoGamepass:CreateButton({
    Name = "🛒 Eksekusi Beli Pancingan Terpilih",
    Callback = function()
        pcall(function()
...
TabPergerakan:CreateSection("✈️ TERBANG & FISIKA")
...
TabPergerakan:CreateToggle({ Name = "Jalan Terpelanting (Lempar Orang)", CurrentValue = false, Callback = function(v) Config.JalanTerpelanting = v end })
TabPergerakan:CreateToggle({ Name = "Anti Terpelanting", CurrentValue = false, Callback = function(v) Config.AntiTerpelanting = v end })

-- [ 5. VISUAL & ESP ]
TabVisual:CreateSection("👁️ SENSOR ESP")
-- Fungsi pembuat ESP Generik
...
TabVisual:CreateSection("☀️ WAKTU & SUASANA")
...
TabNavigasi:CreateSection("🗺️ BASIS DATA KOORDINAT (BAWAAN)")
local BasisLokasi = {
...
-- [ 7. SISTEM ]
TabSistem:CreateSection("🛡️ KEAMANAN")
...
-- [ EFEK RGB BORDER MENU ] (Gabungan dari skrip asli & skrip Hack Rod)
task.spawn(function()
    while true do
...
Rayfield:Notify({ Title = "🔰 Mizukage-Official | TeamMizu", Content = "Ruang kerja premium telah diinisialisasi dengan aman!", Duration = 4, Image = 4483362458 })